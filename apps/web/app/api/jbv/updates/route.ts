...
function buildUrl(searchParams: URLSearchParams) {
  ...
  // Use the view ID so Airtable handles filtering and sorting.
  url.searchParams.set("view", AIRTABLE_VIEW_ID);

  // Support Airtable pagination via offset passthrough.
...
export async function GET(req: Request) {
  ...
  const response = await fetch(url, {
    ...
    // Disable caching during bring-up to avoid stale responses.
    cache: "no-store",
  });
